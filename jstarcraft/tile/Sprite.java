...
import jstarcraft.util.Resource;
import jstarcraft.util.path.AStarNode;
...
public abstract class Sprite extends AbstractTile {
	...
	protected void adjustFight() {
		...
		if (targetX == tileX || targetY == tileY) {
			...
			if (tileX < targetX) {
				currentAnim = tileY < targetY
						? animations[ainmStatus][SOUTH_EAST]
						: animations[ainmStatus][NOTH_EAST];
			}
			else {
				currentAnim = tileY < targetY
						? animations[ainmStatus][SOUTH_WEST]
	...
	public void draw(Graphics2D g, int offsetX, int offsetY) {
		...
		if (isSelected()) {
			...
			g.fillRect(x, y + getHeight() - 5, Math
					.round(getWidth() * (health)), 3);
	...
	public Tile clone(int x, int y, GridMap map) {
		...
		try {
			Sprite sprite = (Sprite) constructor.newInstance(new Object[]{ans,
					id});
